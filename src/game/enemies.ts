...
export function enemyConfig(kind: EnemyKind, tuning: Tuning): Tuning["guard"] | Tuning["stomper"] {
  return kind === "guard" ? tuning.guard : tuning.stomper;
}
...
export function spawnEnemies(room: Room, tuning: Tuning): Enemy[] {
  const out: Enemy[] = [];
  for (const placement of room.enemies) {
    out.push({
      ...
      prev: { x: placement.x, y: placement.y },
      vel: { x: 0, y: 0 },
...
function stepStomper(enemy: Enemy, player: PlayerState, hazards: Hazard[], t: number, tuning: Tuning): void {
  ...
  if (enemy.state === "attack") {
    if (enemy.grounded) {
      ...
      spawnHazard(hazards, enemy.pos.x, y, cfg.waveWidth, cfg.waveHeight, cfg.waveSpeed, life, cfg.damage);
      spawnHazard(hazards, enemy.pos.x, y, cfg.waveWidth, cfg.waveHeight, -cfg.waveSpeed, life, cfg.damage);
    }
    return;
  }
